...
def get_places_files(
    ...
    if ( not (paths.PLACES_CODES_DIR).is_dir() ):
        paths.PLACES_CODES_DIR.mkdir(parents = True, exist_ok = True)
    
    if not (
        len( list(paths.PLACES_CODES_FILES) ) == 
        len( utils.get_files_list(url, census.DATA_URL['PLACES']) )
    ):
        print(message.MISSING_FILE('Places'))
        _download_places(url)
    
    return paths.PLACES_CODES_FILES
...
def _download_places(url: str):
    ...
    for file in files:
        utils.ftp_download_file(
            ...
            filename = file,
            save_to  = paths.PLACES_CODES_FILE(file)
        )
